...
export default function FinnishVocabularyLoading() {
    return (
        <div className="min-h-screen w-full text-slate-900 animate-pulse">
            <div
            ...
            <div className="max-w-9xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex flex-col lg:flex-row gap-12 relative z-10">
                ...
                <main className="flex-1 min-w-0 space-y-16">
                    {/* Hero skeleton */}
                    <div className="flex flex-col md:flex-row items-center gap-8 mb-16 bg-white p-8 sm:p-10 rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                        <div className="flex-1 w-full space-y-4">
                            <div className="h-10 bg-slate-200 rounded-lg w-3/4 max-w-md" />
                            <div className="h-4 bg-slate-100 rounded w-full max-w-xl" />
                            <div className="h-4 bg-slate-100 rounded w-5/6 max-w-lg" />
                        </div>
                        <div className="hidden md:block w-48 h-48 rounded-full bg-slate-100 shrink-0" />
                    </div>
                    ...
                    {[1, 2].map((section) => (
                        <div key={section} className="space-y-6">
                            <div className="flex items-center gap-4">
                                <div className="p-3 rounded-2xl bg-slate-100 w-12 h-12" />
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                {[1, 2, 3, 4, 5, 6].map((i) => (
                                    <div
                                        key={i}
                                        className="h-40 bg-white rounded-3xl border border-slate-200 p-6"
                                    >
