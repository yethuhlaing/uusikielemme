...
export function getByPath(slugSegments: string[] | undefined): WPPostOrPage | null {
  buildPathMap();
  ...
  const match = bySlug.find((p) => linkToPath(p.link).replace(/^\//, "").startsWith(prefix));
...
export function rewriteContentUrls(html: string): string {
  return html
    .replace(new RegExp(WP_ORIGIN_ESCAPED + "/wp-content/uploads/", "g"), UPLOADS_BASE + "/")
